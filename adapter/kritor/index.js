...
class Kritor {
  ...
  /** 收到请求响应 */
  async reverseEvent (data) {
    common.debug('ReverseStream:', data)
    /** debug日志 */
    let rsp = getPbDefByCmd(data.cmd).decode(data.buf)
    common.debug('received data:', rsp)
    // 消费
    waitingMap.get(data.seq)?.({
      raw: data,
      decoded: rsp
    })
  }
  ...
  async LoadBot () {
    ...
    /** 重启 */
    await common.init('Lain:restart:kritor')
    /** 保存uin */
...
common.info('Lain-plugin', 'kritor适配器加载完成')
